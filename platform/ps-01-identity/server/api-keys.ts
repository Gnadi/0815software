...
import type { ApiKeySummary } from '../shared/types.js';
...
export function mintApiKey(
  ...
  const info = db
    .prepare(
      `INSERT INTO api_keys (org_id, name, prefix, key_hash, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
    )
    .run(orgId, name, prefix, hashPassword(secretPart), createdBy, createdAt);
...
/** Verify a presented API key. Returns the owning org id, or null. */
export function verifyApiKey(db: Database.Database, presented: string, now = Date.now()): number | null {
