 ...
 *
 * This module's characteristic misconfiguration is a URL that is slightly
 * wrong. Every other module fails visibly when that happens — it cannot reach
 * its database, and it says so. A shell does not: a mistyped peer URL makes one
 * module render as "not in this stack", which is a perfectly ordinary state
...
describe('defaults', () => {
  ...
  it('reads the port, and refuses one that is not a port', () => {
    ...
    expect(configFromEnv({ PORT: '' }).port).toBe(3016);
...
describe('platform wiring', () => {
  ...
  it('does not read SHELL_ORIGIN, because a shell is not a pane', () => {
    // The three modules that authenticate their own end users leave this
    // unread for the same reason: nothing should frame them. Here the reason
    // is different — a shell framing itself is a recursion with no bottom, and
    // there is no second shell that would want to. The registry says
    // `embeddable: false`, and the drift test holds this file to it.
    expect('shellOrigins' in configFromEnv({ SHELL_ORIGIN: 'https://shell.example/' })).toBe(false);
  });
});
