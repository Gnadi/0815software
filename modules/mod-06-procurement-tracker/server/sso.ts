 ...
 * delegated. When unset, mod-06 falls back to its local admin credentials.
 */
...
export function buildLoginVerifier(cfg: SsoConfig): LoginVerifier {
  ...
  return async (username, password): Promise<LoginOutcome> => {
    ...
    try {
      ...
      if (!(err instanceof ServiceError)) console.warn('[mod-06] identity login unavailable:', err);
      return 'fail';
    }
  };
}
