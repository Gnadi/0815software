...
const app = createApp({ db, hardening: hardeningFromEnv(), auth: config.auth, staticDir, platform: buildPlatform(config.platform), verifyLogin: buildLoginVerifier(config.sso), loginMode: loginModeOf(config.sso) });

app.listen(config.port, () => {
  console.log(`[mod-06] procurement tracker API on http://localhost:${config.port}`);
  if (staticDir) console.log(`[mod-06] serving client from ${staticDir}`);
  if (config.auth.password === 'admin') {
    console.warn('[mod-06] WARNING: using default credentials (admin/admin) — set ADMIN_USERNAME/ADMIN_PASSWORD');
  }
});
