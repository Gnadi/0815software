...
// employees in it is never touched.
seed(db);
...
app.listen(config.port, () => {
  console.log(`[mod-15] absence management API on http://localhost:${config.port}`);
  ...
  if (config.auth.secret === 'dev-secret-change-me' || config.auth.password === 'admin') {
    console.warn('[mod-15] WARNING: using default credentials/secret — set real ones in production');
  }
});
