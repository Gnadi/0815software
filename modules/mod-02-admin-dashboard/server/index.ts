...
// usable immediately. Tables with existing rows are never touched.
seed(db);
...
app.listen(config.port, () => {
  ...
  if (staticDir) console.log(`[mod-02] serving client from ${staticDir}`);
  if (config.auth.password === 'admin') {
    console.warn('[mod-02] WARNING: using default credentials (admin/admin) — set ADMIN_USERNAME/ADMIN_PASSWORD');
  }
});
