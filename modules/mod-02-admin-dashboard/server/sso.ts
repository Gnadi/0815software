 ...
 * delegated. When unset, mod-02 falls back to its local admin credentials.
 */
...
export function buildLoginVerifier(cfg: SsoConfig): LoginVerifier {
  ...
  return async (username, password): Promise<LoginOutcome> => {
    ...
    try {
      ...
      if (!(err instanceof ServiceError)) console.warn('[mod-02] identity login unavailable:', err);
      return { ok: false };
    }
  };
}
