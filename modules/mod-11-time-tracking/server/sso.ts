 ...
 * delegated. When unset, mod-11 falls back to its local admin credentials.
 */
...
export function buildLoginVerifier(cfg: SsoConfig): LoginVerifier {
  ...
  return async (username, password): Promise<LoginOutcome> => {
    ...
    try {
      ...
      if (!(err instanceof ServiceError)) console.warn('[mod-11] identity login unavailable:', err);
      return 'fail';
    }
  };
}
