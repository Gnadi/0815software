 ...
 * in as here (docs/PLATFORM-READINESS.md, item C1). Its summary is counts only: every query in this module is scoped to the
 * signed-in customer, and a shell has no customer.
 */
...
beforeEach(async () => {
  documentsDir = mkdtempSync(join(tmpdir(), 'mod01-summary-'));
  db = openDb(':memory:');
  await seed(db, documentsDir);
  app = createApp({ db, session, documentsDir, serviceToken: TOKEN });
});
...
describe('GET /api/summary — auth', () => {
  ...
  it('is closed when the stack has no machine token', async () => {
    const standalone = createApp({ db, session, documentsDir });
