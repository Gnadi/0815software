 ...
 * entries — and the Workspace renders it as "not installed" rather than as an
...
export interface ServerConfig {
  ...
  databasePath: string;
  /**
   * The Workspace's own public origin. It is what the operator must ALSO set as
   * SHELL_ORIGIN on every embeddable module, and the two are checked against
   * each other at boot so a one-sided change fails loudly.
   */
...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  const port = numberFromEnv('PORT', env.PORT, 3015, 1, 65535);
  return {
    port,
    ...
    platform: {
      ...
      // PS-11 Customers is the party authority. The Workspace reads it to fill
      // the context bar's customer picker — the one identity every module can
      // be narrowed by.
