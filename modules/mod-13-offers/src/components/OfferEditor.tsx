...
interface EditableLine {
  ...
  vatRate: string;
  discount: string; // percent text
}

const EMPTY_LINE: EditableLine = { description: '', quantity: '1', unitPrice: '', vatRate: '20', discount: '0' };
...
export function OfferEditor({ id, onDone, onAuthLost }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (id !== null) {
        const detail = await api.offerDetail(id);
        ...
        setLines(
          detail.lines.map((l) => ({
            ...
            discount: String(l.discount_percent),
          })),
        );
        setLoaded(true);
      }
    ...
    <div>
      ...
      <div className="resource__head">
        <div>
          ...
          <h1 className="resource__title">{id === null ? 'New offer' : `Edit draft #${id}`}</h1>
          <p className="resource__desc">Drafts have no number — one is assigned when you send the offer.</p>
        </div>
      </div>
      ...
      <form className="editor" onSubmit={(e) => void submit(e)}>
        <div className="editor__meta">
          ...
          <label className="field editor__wide">
            <span className="field__label mono">
          ...
          <label className="field editor__wide">
            <span className="field__label mono">INTRODUCTION</span>
            <textarea className="field__input" rows={2} value={intro} onChange={(e) => setIntro(e.target.value)} />
          </label>
          <label className="field editor__wide">
            <span className="field__label mono">TERMS</span>
            <textarea className="field__input" rows={2} value={terms} onChange={(e) => setTerms(e.target.value)} />
          </label>
        </div>

        <div className="section-title mono">LINE ITEMS</div>
        {lines.map((line, i) => (
          <div key={i} className="off-line">
            <label className="field">
              <span className="field__label mono">
            ...
            <label className="field">
              <span className="field__label mono">DISC %</span>
              <input
                className="field__input"
                type="number"
                step="any"
            ...
            <button
              className="rowbtn rowbtn--danger mono off-line__remove"
              type="button"
          ...
          {totals.discount_cents > 0 && (
            <>
              <div className="totals__row mono">
                <span>SUBTOTAL</span>
                <span className="table__num">{fmtEur(totals.net_cents + totals.discount_cents)}</span>
              </div>
              <div className="totals__row totals__row--discount mono">
                <span>LINE DISCOUNTS</span>
                <span className="table__num">{fmtEur(-totals.discount_cents)}</span>
              </div>
            </>
          )}
          <div className="totals__row mono">
