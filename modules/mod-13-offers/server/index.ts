...
// the SELLER_* env is the fallback. Applied before the first request, then
// refreshed periodically so changing it in PS-11 does not need a restart.
// config.seller is mutated in place — createApp captures it and the PDF reads
// its fields at render time.
...
app.listen(config.port, () => {
  console.log(`[mod-13] offers API on http://localhost:${config.port}`);
  if (staticDir) console.log(`[mod-13] serving client from ${staticDir}`);
  console.log(
    usingPlatformSeller
      ? `[mod-13] seller letterhead from PS-11 Customers: ${config.seller.name}`
      : `[mod-13] seller letterhead from SELLER_* env: ${config.seller.name}`,
  );
  if (config.auth.password === 'admin') {
    console.warn('[mod-13] WARNING: using default credentials (admin/admin) — set ADMIN_USERNAME/ADMIN_PASSWORD');
  }
});
