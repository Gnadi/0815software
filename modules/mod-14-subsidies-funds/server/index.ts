...
app.listen(config.port, () => {
  console.log(`[mod-14] subsidies & funds API on http://localhost:${config.port}`);
  if (staticDir) console.log(`[mod-14] serving client from ${staticDir}`);
  if (config.auth.password === 'admin') {
    console.warn('[mod-14] WARNING: using default credentials (admin/admin) — set ADMIN_USERNAME/ADMIN_PASSWORD');
  }
});
