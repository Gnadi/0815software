...
export interface ServerConfig {
  ...
  databasePath: string;
  /**
   * The organisation named on the public intake page. § 17 Abs. 1 HinSchG
   * requires the reporter to know who is receiving their report.
   */
  organisation: string;
  /** Per-IP limit on access-code lookups per minute. See server/throttle.ts. */
...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  return {
    port: numberFromEnv('PORT', env.PORT, 3016, 1, 65535),
    databasePath: env.DATABASE_PATH ?? './data.db',
    organisation: env.ORGANISATION_NAME ?? 'Example GmbH',
    lookupRateLimitRpm: numberFromEnv('LOOKUP_RATE_LIMIT_RPM', env.LOOKUP_RATE_LIMIT_RPM, 20, 0, 100000),
    auth: {
