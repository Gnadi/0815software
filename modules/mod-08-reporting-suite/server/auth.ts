...
export const COOKIE_NAME = 'mod08_session';
...
/**
 * Embed tokens sign a specific chart id, so a token minted for chart 3
 * only ever renders chart 3. They carry no expiry — an embed URL is a
 * long-lived capability; rotate SESSION_SECRET to revoke all of them.
 */
export function createEmbedToken(secret: string, chartId: number): string {
  return sign(`embed:chart:${chartId}`, secret);
}

export function verifyEmbedToken(secret: string, chartId: number, token: unknown): boolean {
  if (typeof token !== 'string' || token.length === 0) return false;
  return safeEqual(token, createEmbedToken(secret, chartId));
}
