...
| --- | --- | --- |
| **PS-01 Identity** | User email, name, password hash, org membership, auth-event IPs | `POST /api/users/:id/erase` anonymizes email/name, scrambles the password, bumps `token_version` (kills live sessions), **revokes every API key that person minted** and disables the account, keeping the row+id for referential integrity. Records a `user_erased` audit event carrying the number of keys revoked. The keys matter: one is verified against its own hash and carries the permissions its creator held — `platform:admin` among them — so leaving them live left the erased person's authority working after every other door had been locked. A merely *disabled* account keeps its keys, deliberately: a suspension is reversible and a service account should not stop mid-pipeline. `GET /api/api-keys` reports `created_by` so an operator can see which keys a leaver minted before deciding. |
